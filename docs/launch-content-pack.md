...
8. Use `docs/assets/terminal-output-demo.svg` only after
   `npm run check:terminal-output-demo` passes on the same commit.
9. Use `docs/assets/precommit-rejection-demo.svg` only after
   `npm run check:precommit-rejection-demo` passes on the same commit.
10. Do not mention stars, followers, installs, traffic, or engagement unless a
   current source is attached.
11. Run `npm run check:launch-content` before approving any launch copy changes.
