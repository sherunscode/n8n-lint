...
export const bundledN8nPackageVersions = Object.keys(bundledN8nPackageConfig.selections).sort((left, right) => left.localeCompare(right));
export const defaultBundledN8nPackageVersion = bundledN8nPackageConfig.defaultPackageVersion;
const defaultBundledSchemaArtifactPath = join(dirname(fileURLToPath(import.meta.url)), "../schema/bundled-n8n-package.json");
const bundledSchemaArtifactPaths = Object.fromEntries(bundledN8nPackageVersions.map((packageVersion) => {
    const selection = readBundledN8nPackageConfigSelection(packageVersion);
    const artifactPath = packageVersion === defaultBundledN8nPackageVersion
...
export const bundledN8nPackageSelections = Object.fromEntries(Object.entries(bundledN8nPackageConfig.selections).map(([packageVersion, selection]) => [
    packageVersion,
...
function readBundledN8nPackageConfig() {
    ...
    const credentialMetadataFile = readNonEmptyConfigString(parsed.credentialMetadataFile, "credentialMetadataFile", configPath);
    const schemaDirectory = readNonEmptyConfigString(parsed.schemaDirectory, "schemaDirectory", configPath);
    const defaultPackageVersion = readNonEmptyConfigString(parsed.defaultPackageVersion, "defaultPackageVersion", configPath);
    if (!isRecord(parsed.selections)) {
        throw new Error(`Bundled n8n package config selections must be an object at ${configPath}.`);
    }
    const selections = Object.fromEntries(Object.entries(parsed.selections).map(([packageVersion, value]) => [
        packageVersion,
...
function readConfigSelection(packageVersion, value, packageName, configPath) {
    ...
    const selection = {
        referencePackage: readNonEmptyConfigString(value.referencePackage, `${packageVersion}.referencePackage`, configPath),
        ...
        workflowPackageVersion: readNonEmptyConfigString(value.workflowPackageVersion, `${packageVersion}.workflowPackageVersion`, configPath),
...
function readBundledN8nPackageConfigSelection(packageVersion) {
...
function toPublicBundledN8nPackageSelection(selection) {
    return {
...
function readNonEmptyConfigString(value, label, configPath) {
    if (typeof value !== "string" || value.trim() === "") {
        throw new Error(`Bundled n8n package config ${label} is invalid at ${configPath}.`);
    }
    return value.trim();
}
export function createLocalPlaceholderSchemaSource() {
    return {
...
export function createBundledN8nPackageSchemaSource(config = {}) {
    const artifactPath = config.artifactPath ?? bundledSchemaArtifactPaths[config.packageVersion ?? defaultBundledN8nPackageVersion];
    ...
    return {
        ...
        async load() {
            ...
            const packageInfo = artifact.package;
...
export function createLiveRestSchemaSource(config) {
    return {
        kind: "live-rest",
        load() {
...
function validateLiveRestBaseUrl(rawBaseUrl) {
...
export function isBundledN8nPackageVersion(value) {
    return bundledN8nPackageVersions.includes(value);
}
async function readBundledSchemaArtifact(filePath) {
    ...
    const nodeParameterNames = parsed.schemaVersion === 1 ? {} : readStringArrayRecord(parsed.nodeParameterNames, "nodeParameterNames", filePath);
    const nodeParameterPaths = parsed.schemaVersion === 3 ? readStringArrayRecord(parsed.nodeParameterPaths, "nodeParameterPaths", filePath) : {};
    const triggerNodeTypes = parsed.schemaVersion === 1 ? [] : readStringArray(parsed.triggerNodeTypes, "triggerNodeTypes", filePath);
...
function readArtifactSelection(value, packageInfo, filePath) {
    if (!isRecord(value)) {
    ...
    const workflowPackageVersion = readNonEmptySelectionString(value.workflowPackageVersion, "workflowPackageVersion", filePath);
...
function inferArtifactSelection(packageInfo, filePath) {
...
function readNonEmptySelectionString(value, label, filePath) {
    if (typeof value !== "string" || value.trim() === "") {
        throw new Error(`Bundled n8n schema artifact selection.${label} is invalid at ${filePath}.`);
    }
    return value.trim();
}
function readArtifactPackageInfo(value, filePath) {
...
function readStringArray(rawValue, label, filePath) {
...
function readStringArrayRecord(rawValue, label, filePath) {
