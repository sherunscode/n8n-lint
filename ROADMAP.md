...
- Local quality gates prove build, lint, format, fixtures, metadata, security
  ...
  CLI output color/no-color behavior, community readiness, release readiness,
  live REST boundary enforcement, launch-content proof, benchmark-report
  reconciliation, GitHub Action contract, strategy-checklist reconciliation,
  README demo freshness, animated-demo freshness, social-preview freshness,
  audit-report freshness,
  superseded-status note clarity, README/CLI docs contract, package contents,
  claims hygiene, Markdown link integrity, executable exit-code behavior, tests,
  and production dependency audit status.
...
- Release readiness is executable, with npm publish still behind an owner gate.
